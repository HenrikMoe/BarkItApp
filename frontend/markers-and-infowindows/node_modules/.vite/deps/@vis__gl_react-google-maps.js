...
function useGoogleMapsApiLoader(props) {
  ...
  const serializedParams = (0, import_react.useMemo)(() => JSON.stringify(otherApiParams), [otherApiParams]);
    ...
    () => {
      (async () => {
        try {
          ...
          }, otherApiParams), (status2) => setStatus(status2));
...
function useMapOptions(map, cameraStateRef, mapProps) {
  ...
  (0, import_react.useLayoutEffect)(() => {
    if (!map || !Number.isFinite(lat) || !Number.isFinite(lng))
      return;
    if (cameraStateRef.current.center.lat === lat && cameraStateRef.current.center.lng === lng)
      return;
    map.moveCamera({
      center: {
        lat,
  ...
  (0, import_react.useLayoutEffect)(() => {
    if (!map || !Number.isFinite(zoom))
      return;
    if (cameraStateRef.current.zoom === zoom)
      return;
    map.moveCamera({
      zoom
    });
  ...
  (0, import_react.useLayoutEffect)(() => {
    if (!map || !Number.isFinite(heading))
      return;
    if (cameraStateRef.current.heading === heading)
      return;
    map.moveCamera({
      heading
    });
  ...
  (0, import_react.useLayoutEffect)(() => {
    if (!map || !Number.isFinite(tilt))
      return;
    if (cameraStateRef.current.tilt === tilt)
      return;
    map.moveCamera({
      tilt
    });
...
var useAutocomplete = (props) => {
  ...
  (0, import_react.useEffect)(() => {
    ...
    autocompleteInstance.addListener("place_changed", () => {
      ...
      if (placeChangedHandler.current)
